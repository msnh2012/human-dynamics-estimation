...
public:
    ...
    iDynTree::SparseMatrix<iDynTree::ColumnMajor>
        priorDynamicsConstraintsCovarianceInverse; // Sigma_D^-1
    iDynTree::SparseMatrix<iDynTree::ColumnMajor>
        priorDynamicsRegularizationCovarianceInverse; // Sigma_d^-1
    ...
    static bool invertSparseMatrix(const iDynTree::SparseMatrix<iDynTree::ColumnMajor>& in,
...
void BerdySparseMAPSolver::setDynamicsConstraintsPriorCovariance(
    const iDynTree::SparseMatrix<iDynTree::ColumnMajor>& covariance)
{
    ...
    BerdySparseMAPSolver::BerdySparseMAPSolverPimpl::invertSparseMatrix(
        covariance, m_pimpl->priorDynamicsConstraintsCovarianceInverse);
}

void BerdySparseMAPSolver::setDynamicsRegularizationPriorCovariance(
    const iDynTree::SparseMatrix<iDynTree::ColumnMajor>& covariance)
{
    assert(m_pimpl);
    assert(covariance.rows() == m_pimpl->priorDynamicsRegularizationCovarianceInverse.rows()
           && covariance.columns()
                  == m_pimpl->priorDynamicsRegularizationCovarianceInverse.columns());
    BerdySparseMAPSolver::BerdySparseMAPSolverPimpl::invertSparseMatrix(
        covariance, m_pimpl->priorDynamicsRegularizationCovarianceInverse);
}

void BerdySparseMAPSolver::setDynamicsRegularizationPriorExpectedValue(
    const iDynTree::VectorDynSize& expectedValue)
{
    assert(m_pimpl);
...
void BerdySparseMAPSolver::setMeasurementsPriorCovariance(
    const iDynTree::SparseMatrix<iDynTree::ColumnMajor>& covariance)
{
    assert(m_pimpl);
    ...
    BerdySparseMAPSolver::BerdySparseMAPSolverPimpl::invertSparseMatrix(
        covariance, m_pimpl->priorMeasurementsCovarianceInverse);
}

const iDynTree::SparseMatrix<iDynTree::ColumnMajor>&
BerdySparseMAPSolver::dynamicsConstraintsPriorCovarianceInverse() const
{
    assert(m_pimpl);
...
iDynTree::SparseMatrix<iDynTree::ColumnMajor>&
BerdySparseMAPSolver::dynamicsConstraintsPriorCovarianceInverse()
{
    assert(m_pimpl);
    return m_pimpl->priorDynamicsConstraintsCovarianceInverse;
}
...
const iDynTree::SparseMatrix<iDynTree::ColumnMajor>&
BerdySparseMAPSolver::dynamicsRegularizationPriorCovarianceInverse() const
{
    assert(m_pimpl);
...
iDynTree::SparseMatrix<iDynTree::ColumnMajor>&
BerdySparseMAPSolver::dynamicsRegularizationPriorCovarianceInverse()
{
    assert(m_pimpl);
    return m_pimpl->priorDynamicsRegularizationCovarianceInverse;
}
...
BerdySparseMAPSolver::dynamicsRegularizationPriorExpectedValue() const
{
    assert(m_pimpl);
...
const iDynTree::SparseMatrix<iDynTree::ColumnMajor>&
BerdySparseMAPSolver::measurementsPriorCovarianceInverse() const
{
    assert(m_pimpl);
...
iDynTree::SparseMatrix<iDynTree::ColumnMajor>&
BerdySparseMAPSolver::measurementsPriorCovarianceInverse()
{
    assert(m_pimpl);
    return m_pimpl->priorMeasurementsCovarianceInverse;
}
...
void BerdySparseMAPSolver::updateEstimateInformationFixedBase(
    ...
    const iDynTree::VectorDynSize& measurements)
{
    ...
    m_pimpl->berdy.updateKinematicsFromFixedBase(
        m_pimpl->jointsConfiguration, m_pimpl->jointsVelocity, fixedFrame, gravity);
}

void BerdySparseMAPSolver::updateEstimateInformationFloatingBase(
    const iDynTree::JointPosDoubleArray& jointsConfiguration,
    const iDynTree::JointDOFsDoubleArray& jointsVelocity,
    ...
    const iDynTree::VectorDynSize& measurements)
{
    ...
    m_pimpl->berdy.updateKinematicsFromFloatingBase(m_pimpl->jointsConfiguration,
...
void BerdySparseMAPSolver::BerdySparseMAPSolverPimpl::computeMAP(bool computePermutation)
{
     ...
     * Get berdy matrices
     */
    berdy.getBerdyMatrices(
        dynamicsConstraintsMatrix, dynamicsConstraintsBias, measurementsMatrix, measurementsBias);
    ...
    covarianceDynamicsPriorInverse += toEigen(dynamicsConstraintsMatrix).transpose()
                                      * toEigen(priorDynamicsConstraintsCovarianceInverse)
                                      * toEigen(dynamicsConstraintsMatrix);
    ...
    toEigen(expectedDynamicsPriorRHS) = toEigen(priorDynamicsRegularizationCovarianceInverse)
                                            * toEigen(priorDynamicsRegularizationExpectedValue)
                                        - toEigen(dynamicsConstraintsMatrix).transpose()
                                              * toEigen(priorDynamicsConstraintsCovarianceInverse)
                                              * toEigen(dynamicsConstraintsBias);
    ...
    covarianceDynamicsAPosterioriInverse += toEigen(measurementsMatrix).transpose()
                                            * toEigen(priorMeasurementsCovarianceInverse)
                                            * toEigen(measurementsMatrix);
    ...
    if (computePermutation) {
        ...
        covarianceDynamicsAPosterioriInverseDecomposition.analyzePattern(
            covarianceDynamicsAPosterioriInverse);
    }
    ...
    covarianceDynamicsAPosterioriInverseDecomposition.factorize(
        covarianceDynamicsAPosterioriInverse);

    // Final result: expected value of the whole-body dynamics, Eq. 11b
    toEigen(expectedDynamicsAPosterioriRHS) =
        (toEigen(measurementsMatrix).transpose() * toEigen(priorMeasurementsCovarianceInverse)
             * (toEigen(measurements) - toEigen(measurementsBias))
         + covarianceDynamicsPriorInverse * toEigen(expectedDynamicsPrior));
    toEigen(expectedDynamicsAPosteriori) = covarianceDynamicsAPosterioriInverseDecomposition.solve(
        toEigen(expectedDynamicsAPosterioriRHS));
}
...
bool BerdySparseMAPSolver::BerdySparseMAPSolverPimpl::initialize()
{
    ...
    iDynTree::Vector3 initialGravity;
    initialGravity.zero();
    initialGravity(2) = -9.81;

    berdy.updateKinematicsFromFixedBase(jointsConfiguration,
                                        jointsVelocity,
                                        berdy.dynamicTraversal().getBaseLink()->getIndex(),
                                        initialGravity);
    computeMAP(true);
...
bool BerdySparseMAPSolver::BerdySparseMAPSolverPimpl::invertSparseMatrix(
    const iDynTree::SparseMatrix<iDynTree::ColumnMajor>& in,
    iDynTree::SparseMatrix<iDynTree::ColumnMajor>& inverted)
{
    Eigen::SimplicialLDLT<Eigen::SparseMatrix<double, Eigen::ColMajor>> inversion;
    inversion.compute(iDynTree::toEigen(in));
    if (inversion.info() != Eigen::Success)
        return false;
